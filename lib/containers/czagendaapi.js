var util = require('util'),
    ...
    Node = require('czagenda-cluster').containers.Node,
...
CzagendaApi.prototype.installServer = function(callback) {
  ...
  this.exec( [
      ...
      'cat settings.sample.js|sed "s/%IP%/'+this.getIp()+'/g" | sed "s/%ES_IP%/'+ settings.ELASTICSEARCH_FAILOVER +'/g" | sed "s/%REDIS_IP%/'+ settings.REDIS_FAILOVER +'/g" | sed "s/%ES_INDEX%/'+ settings.ELASTICSEARCH_INDEX_NAME +'/g" > settings.js',
    	'mkdir pids',
      'mkdir logs',
      'npm update',
...
CzagendaApi.prototype.updateServer = function(callback) {
  this.exec( [
    'cd /home/czagenda-api',
    'git pull',
    'cat settings.sample.js|sed "s/%IP%/'+this.getIp()+'/g" | sed "s/%ES_IP%/'+ settings.ELASTICSEARCH_FAILOVER +'/g" | sed "s/%REDIS_IP%/'+ settings.REDIS_FAILOVER +'/g" > settings.js',
    'npm update'
  ].join(' && '), null, function(err,res) {
    if(err) {
      callback(err);
